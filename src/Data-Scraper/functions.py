...
def openMta(user, windows, selenBrowser):
    ...
    username = selenBrowser.find_element_by_name("user")
    password = selenBrowser.find_element_by_name("pw")
    submit = selenBrowser.find_element_by_xpath('//input[@type="submit"]')

    wait(5)
    username.click()
    wait(3)
    username.send_keys(user.get_mta_username())
    wait(4)
    password.click()
    wait(4)
    password.send_keys(user.get_mta_password())
    wait(4)
    submit.click()
    wait(5)
